...
In this part, the table containing the reads for each individuals (in column) per taxons (in rows) is treated. We keep only the column of interests and the rows containing the bests informations. This program returns two tables: one containing the different taxons and one containing the differents individuals.
...
```{r}
write.table(subset(final_df), 
            file = "../refined_data/filtered_taxonomic_abundance.csv",
            col.names = TRUE,
            row.names = FALSE,
            sep=";")

write.table(subset(abundance_df), 
