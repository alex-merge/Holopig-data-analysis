...
We can also filter the abundance, keeping only those that have at least one that is more than 1%.

```{r}
unfiltered_abundance = abundance_df
abundance_df$sum = rowSums(abundance_df[9:42])

thres = 0.01
abundance_df = subset(abundance_df, sum >= thres)
```
...
write.table(abundance_df, 
            file = "../refined_data/filtered_phylogenic_X_sample.csv",
            col.names = TRUE,
            row.names = FALSE,
            sep=";")
