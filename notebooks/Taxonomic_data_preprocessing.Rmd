...
## Retrieving contigs for each rows
library(tidyr)
contigs_col = c(paste0("name_contigs_Holopig_colon0",seq(9),"_quantif_percontig"),
                paste0("name_contigs_Holopig_colon",seq(10,34),"_quantif_percontig"))

tmp_df = raw_df[,contigs_col]
colnames(tmp_df) = paste0("piglet_", seq(1,34))
tmp_df[tmp_df == 0] = NA
abundance_df$contigs = (tmp_df %>% unite("contigs", piglet_1:piglet_34, sep = ";", na.rm = T))$contigs
colnames(abundance_df)[ncol(abundance_df)] = "contigs"
abundance_df$contigs = gsub(";", "\t", abundance_df$contigs)
abundance_df$contigs = gsub("Holopig_colon", "", abundance_df$contigs)
abundance_df$contigs = gsub("_", "", abundance_df$contigs)

# Removing unnecessary variables
rm(list= c("col_name", "sample_number", "tmp_df"))
```
...
```{r}
base_taxo = abundance_df[, c(1:8, ncol(abundance_df)-1)]
...
# identification of the individual
  tmp$piglet = as.factor(rep(sample, dim(base_taxo)[1]))
# identification of the group
  tmp$group = as.factor(rep(category[sample], dim(base_taxo)[1]))
# relative abundance of the species if the individual
  tmp$abund = abundance_df[[8+sample]]
...
```{r}
write.table(subset(final_df, select = -contigs), 
            file = "../refined_data/filtered_taxonomic_abundance.csv",
            col.names = TRUE,
            row.names = FALSE,
            sep=";")

write.table(subset(abundance_df, select = -contigs), 
            file = "../refined_data/filtered_taxonomy_X_sample.csv",
            col.names = TRUE,
            row.names = FALSE,
            sep=";")

write.table(subset(unfiltered_abundance, select = -contigs), 
            file = "../refined_data/complete_taxonomy_X_sample.csv",
            col.names = TRUE,
            row.names = FALSE,
            sep=";")

write.table(subset(unfiltered_abundance, 
                   select = c(root, kingdom, phylum, class, order, family, genus, species, contigs)),
            file = "../refined_data/complete_taxonomy_X_contigs.csv",
            col.names = TRUE,
            row.names = FALSE,
            sep=";")
```
