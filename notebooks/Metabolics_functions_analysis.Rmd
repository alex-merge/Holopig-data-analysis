...
-   `category` : class of the piglet. The vector begins with the category of the firs piglet, the last category given corresponds to the last piglet (here the 34th piglet) .
...
```{r}
rm(list = c("category", "columns_reads", "file", "filepath", "name_pig", "nb_pig", "rows_reads", "sum_reads_pig", "transposed_cat_pig"))
```
...
columns_filtered = c(name_sample, "COG_category", "GOs", "EC", "KEGG_ko", "KEGG_Pathway", "KEGG_Module", "KEGG_Reaction", "CAZy", "PFAMs")
...
for (file in directory) {
  ...
  data = data %>% select(-c(seed_cluster, seed_ortholog, evalue, score, eggNOG_OGs, max_annot_lvl, Description, Preferred_name, KEGG_rclass, BRITE, KEGG_TC, BiGG_Reaction, sseqid, stitle))
...
write.table(data_filtered, 
            file = "../refined_data/Quantifications_and_functional_annotations_filtered.csv", 
            sep = ';', 
            row.names = FALSE, 
            quote = FALSE)
...
## Getting a data frame for the selected function

All the processing before needs to be run one time. The following processing steps can be run several times (for each function of interest) one the same `data_filtered` data frame because all the possible functions were kept.
...
```{r}
#choose the function of interest, several choices but only one at the time

#function_interest = c("COG_category")    #Working
#function_interest = c("GOs")             #Working
#function_interest = c("KEGG_ko")         #Working
function_interest = c("KEGG_Pathway")    #Working
#function_interest = c("KEGG_Module")     #Working
#function_interest = c("KEGG_Reaction")   #Working
#function_interest = c("CAZy")            #Working
#function_interest = c("PFAMs")
#function_interest = c("EC")              #Working
...
The COG_category creates empty cell (because of the raw data) and we regroup the enzymes by their class and not by their names.

```{r}

if (function_interest == "COG_category") {
  functions_interest_data_sum = separate_rows(functions_interest_data, all_of( function_interest), sep = "", convert = TRUE)
  ...
  functions_interest_data_sum[[function_interest]] = str_remove(functions_interest_data_sum[[function_interest]], "([:punct:][:digit:]*)$")

} else if (function_interest == "GOs" | function_interest == "KEGG_ko" | function_interest == "KEGG_Pathway" | function_interest == "KEGG_Reaction" | function_interest == "CAZy" | function_interest == "PFAMs" | function_interest == "KEGG_Module") {
  functions_interest_data_sum = separate_rows(functions_interest_data,all_of( function_interest), sep = ",", convert = TRUE)
}
...
write.table(functions_interest_final_dataframe, 
            file = paste("../refined_data/Function_", function_interest, "_dataframe.csv", sep = ""), 
            sep = ';', 
            row.names = FALSE, 
            quote = FALSE)
```

### Ordering functions per the mean of the relative abundance

We select the 100th first higher relative abundances for the functions where there are more than 100 items. Exemple : COG_category only contains 24 items max whereas EC can contain oven 6000 items.

```{r}

if (nrow(functions_interest_final_dataframe > 100)) {
  
  functions_interest_final_dataframe_ordered = order(rowMeans(functions_interest_final_dataframe[,-1]), decreasing = TRUE)
  functions_interest_final_dataframe = functions_interest_final_dataframe[functions_interest_final_dataframe_ordered[1:100],]
...
```{r}
rm(list = c("functions_interest_data","functions_interest_data_sum","functions_interest_final_dataframe_ordered"))
```

## Plotting heat maps for the selected functions

### Importing the database for the correspondence

We can use a database to get the correspondence between the code in the data to a meaningful name. The user can choose which database will be use for a selected function.

**WARNING** : The database must be shape in 2 columns, the first one must be the `Code_function` and the second one must be the `Definition_function`.
...
if (function_interest == "COG_category")  {
  ...
  functions_name_corresp = read.csv("../ressources/EC_equivalence_classes.csv", sep = ";")
...
Now we rename the piglet for a shorter name, make the correspondence with the database (if it was given previously) and rename functions with NA (former "-" functions).

```{r}

reshaped_functions = reshaped_functions %>% mutate(Pig_name = gsub("(\\D)*", "", Pig_name))

if (exists("functions_name_corresp") == TRUE) {
...
If the user wants to re-run the production of heat maps, he needs to run the following command and go back to the point [Getting a data frame for the selected function].
