...
metabo_gene_thres = melted_data %>% subset(melted_data$value <= -thres_keep | melted_data$value >= thres_keep)
colnames(metabo_gene_thres) = c("metabolite", "gene", "value")

head(metabo_gene_thres)
...
print(paste("Number of correlation with a value inferior to the inversed threshold (", -thres_keep, ") : ", sum(metabo_gene_thres$value <= -thres_keep)))

# ggplot(melted_data, aes(x=value)) + 
#   geom_histogram(binwidth=0.10) +
#   geom_vline(xintercept = thres_keep, linetype = "dashed", linewidth = 1.5, 
#              color = "red")+
...
#   ggtitle("Distribution of correlation values")
...
filtered_melted_data = metabo_gene_thres
