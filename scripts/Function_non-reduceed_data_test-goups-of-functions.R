...
columns = c("COG_category", name_sample, "Sum")                                      #MODIFIED
...
for (file in directory) {
  ...
  functions_table_file = data.frame(data$COG_category, data[, grepl(".featureCounts.tsv$", names(data))], data$sum)          #MODIFIED
  colnames(functions_table_file)[colnames(functions_table_file) == 'data.COG_category'] <- 'COG_category'                    #MODIFIED
  ...
  functions_light_table = aggregate(functions_table_file[, colnames(functions_table_file)[colnames(functions_table_file) != 'COG_category']], list(functions_table_file$COG_category), FUN=sum)      #MODIFIED
  colnames(functions_light_table)[colnames(functions_light_table) == 'Group.1'] <- 'COG_category'                  #MODIFIED
  ...
  functions_general_table = aggregate(functions_general_table[, colnames(functions_general_table)[colnames(functions_general_table) != 'COG_category']], list(functions_general_table$COG_category), FUN=sum)        #MODIFIED
  colnames(functions_general_table)[colnames(functions_general_table) == 'Group.1'] <- 'COG_category'               #MODIFIED
...
colnames(reshaped_functions) = c("COG_category","Relative_abundance","Pig_name")

for (name_pig in name_sample) {
  description = functions_general_abdrev$COG_category
  ...
  reshaped_functions = merge(reshaped_functions, reshaped_functions_pig, all = TRUE, by.y = c("description","relative_abundance","pig"), by.x = c("COG_category","Relative_abundance","Pig_name"))
}
...
ggplot(reshaped_functions, aes(Pig_name, COG_category), fill= Relative_abundance) +
  geom_tile()

# Heatmap ggplot
ggplot(reshaped_functions, aes(Pig_name,COG_category)) + 
  ...
  facet_grid(cols = vars(factor(category, levels = c("Control", "Colistine"))),
             ...
             space = "free")
scale = 200
ggsave(filename = "export/heatmap_metabolic_functions_test_bis.png",
       width = 16*scale,
       height = 18*scale,
       units = "px",
       dpi=200)
